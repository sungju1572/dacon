...
#train에서 공급유형 공공분양인것들 제거 (C1350)
idx_drop = train[train["공급유형"]=="공공분양"].index

train = train.drop(idx_drop)

train = train.reset_index(drop=True)
#test 에서 아파트인데 임대보증금, 임대료 없는 컬럼 제거 (C1006)
test = test.drop([test.index[11],test.index[12],test.index[14]])
...
#스케일러
scaler = RobustScaler()
scaler.fit(group_train)

print("meadian : ", scaler.center_) #원래 데이터의 중앙값
print("IQR : ", scaler.scale_) #IQR 값


#train
group_train = pd.DataFrame(scaler.fit_transform(group_train))
...
#test
group_test = pd.DataFrame(scaler.fit_transform(group_test))
...
#스케일러
data_standadized_skl = StandardScaler().fit_transform(group_train)



#train
group_train = pd.DataFrame(data_standadized_skl)
...
#test
group_test = pd.DataFrame(StandardScaler().fit_transform(group_test))
...
math.log(group_train["label"])

for i in range(len(group_train["label"])):
    group_train["label"][i] = math.log(group_train["label"][i],10)
"""
