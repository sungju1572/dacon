...
#age 데이터프레임에서 지역별 상위 5개 뽑기
age_over20 = age.iloc[:,5:]


for i in range(len(age_over20)):
    a = age_over20.loc[i].rank(ascending=False)
    for j in range(len(a)):
        if a[j] <=5:
            age_over20.loc[i][j] = a[j]
        else:
            age_over20.loc[i][j] = 0
           
            

for i in range(len(age_over20)):
    for j in range(len(age_over20.columns)):
        if age_over20.iloc[i,j] == 1:
            age_over20.iloc[i,j] = 5
        elif age_over20.iloc[i,j] == 2:
            age_over20.iloc[i,j] = 4
        elif age_over20.iloc[i,j] == 4:
            age_over20.iloc[i,j] = 2
        elif age_over20.iloc[i,j] ==5:
            age_over20.iloc[i,j] = 1
            
            
age_over = age_over20.iloc[:,:11]



age_over.info()

age_over["지역_1"] = age["지역"]


group_train[age_over.columns] = 0
group_train = group_train.drop("지역_1", axis=1)


group_test[age_over.columns] = 0
group_test = group_test.drop("지역_1", axis=1)

#지역별 랭킹 합치기
#train
for i in range(len(group_train)):
   for j in range(len(age_over)):
       if group_train["지역"][i] == age_over["지역_1"][j]:
              group_train["20대(여자)"][i] = age_over["20대(여자)"][j]
              group_train["20대(남자)"][i] = age_over["20대(남자)"][j]
              group_train["30대(여자)"][i] = age_over["30대(여자)"][j]
              group_train["30대(남자)"][i] = age_over["30대(남자)"][j]
              group_train["40대(여자)"][i] = age_over["40대(여자)"][j]
              group_train["40대(남자)"][i] = age_over["40대(남자)"][j]
              group_train["50대(여자)"][i] = age_over["50대(여자)"][j]
              group_train["50대(남자)"][i] = age_over["50대(남자)"][j]
              group_train["60대(여자)"][i] = age_over["60대(여자)"][j]
              group_train["60대(남자)"][i] = age_over["60대(남자)"][j]
              group_train["70대(여자)"][i] = age_over["70대(여자)"][j]
...
for i in range(len(group_test)):
   for j in range(len(age_over)):
       if group_test["지역"][i] == age_over["지역_1"][j]:
              group_test["20대(여자)"][i] = age_over["20대(여자)"][j]
              group_test["20대(남자)"][i] = age_over["20대(남자)"][j]
              group_test["30대(여자)"][i] = age_over["30대(여자)"][j]
              group_test["30대(남자)"][i] = age_over["30대(남자)"][j]
              group_test["40대(여자)"][i] = age_over["40대(여자)"][j]
              group_test["40대(남자)"][i] = age_over["40대(남자)"][j]
              group_test["50대(여자)"][i] = age_over["50대(여자)"][j]
              group_test["50대(남자)"][i] = age_over["50대(남자)"][j]
              group_test["60대(여자)"][i] = age_over["60대(여자)"][j]
              group_test["60대(남자)"][i] = age_over["60대(남자)"][j]
              group_test["70대(여자)"][i] = age_over["70대(여자)"][j]
              
"""
group_train.iloc[:,17:] = group_train.iloc[:,17:].astype("uint8")                      

group_test.iloc[:,16:] = group_test.iloc[:,16:].astype("uint8")   
"""
...
aa = [xgb.feature_importances_]



aa.rank(ascending=False)
"""
